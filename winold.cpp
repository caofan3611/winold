...
winold::winold_pen winold::pen;
    ...
    bool winold::isError(bool s){
        debug("进入isError(bool s)");
        debug("s:",s);
        debug("结果是:",s==WINOLD_ERROR_BOOL);
       return s==WINOLD_ERROR_BOOL;
    }
...
bool winold::replacePic(string s,vector<string> v){
    winold_error err;
    if(pics.empty()){
...
bool winold::replacePic(long long l,vector<string> v){
    closeError();
    winold_error err;
    auto i=getPicName(l);
    if(isError(i))
