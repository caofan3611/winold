...
ll winold::playVid(ll l,ll times){
    ...
    for(ll i=0;i<vids[i].thing.size();i++) {
        cout <<vids[i].thing[i]<<std::flush;
        std::this_thread::sleep_for(std::chrono::microseconds(vids[i].sleepn));
        clearScreen();
    }
    }
    return 0;
